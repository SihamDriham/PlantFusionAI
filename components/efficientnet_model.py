...
@st.cache_resource
def load_efficientnet_multimodal_model(checkpoint_path="models/multimodal/best_model_efficientnet.pth"):
    """
    Charge le modèle EfficientNet multimodal pour la fusion précoce
    """
    ...
    class EfficientNetMultimodal(nn.Module):
        def __init__(self, in_channels, num_classes):
            ...
            if in_channels != 3:
                original_weight = self.efficientnet.features[0][0].weight.data.clone()
                new_conv = nn.Conv2d(
                    in_channels, 
                    32,
                    kernel_size=3, 
                ...
                with torch.no_grad():
                    ...
                    if in_channels > 3:
                        nn.init.normal_(new_conv.weight.data[:, 3:], 
                                  mean=0.0, 
    ...
    try:
        ...
        if 'model_state_dict' in checkpoint:
            ...
            # Détecter le nombre de canaux d'entrée
            for key in state_dict.keys():
                if 'features.0.0.weight' in key or 'efficientnet.features.0.0.weight' in key:
                    in_channels = state_dict[key].shape[1]
                    break
            
            # Détecter le nombre de classes
            for key in state_dict.keys():
                if 'classifier.1.weight' in key or 'efficientnet.classifier.1.weight' in key:
        ...
        # Créer le modèle
        model = EfficientNetMultimodal(in_channels=in_channels, num_classes=num_classes)
        ...
        st.info(f"EfficientNet chargé avec {in_channels} canaux d'entrée et {num_classes} classes")
        return model
        
    except Exception as e:
        import traceback
        st.error(f"Erreur lors du chargement du modèle EfficientNet: {e}")
        st.error(f"Détails: {traceback.format_exc()}")
        return None

def preprocess_early_fusion_efficientnet(rgb_image, ms_images, ms_channels=3):
    """
    Prétraite une image RGB et un ensemble d'images multispectrales pour l'entrée du modèle EfficientNet
    ...
    """
      # Taille standard pour EfficientNet-B0
    ...
    st.info(f"Tensor préparé pour EfficientNet: {input_tensor.shape}")
    return input_tensor


def predict_with_efficientnet(model, input_tensor):
    """
    Fait une prédiction avec le modèle EfficientNet
